...
const PREFIX = process.env.PREFIX || 'l!';
...
client.once(Events.ClientReady,bot=>console.log(`🦁 Leonis online como ${bot.user.tag} • ${client.commands.size} comandos • prefixo ${PREFIX}`));
client.on(Events.MessageCreate,async m=>{if(!m.guild||m.author.bot)return;const c=getGuildConfig(m.guild.id);if(c.security.blockInvites&&isBlockedInvite(m.content)){try{await m.delete();}catch{}recordMessage(m,'blocked-invite');return}if(c.security.antiFlood){const now=Date.now(),key=`${m.guild.id}:${m.author.id}`,e=(recentMessages.get(key)||[]).filter(t=>now-t<8000);e.push(now);recentMessages.set(key,e);if(e.length>=c.security.floodThreshold)try{await m.member.timeout(10000,'Leonis anti-flood')}catch{}}recordMessage(m,'created');
  if(!m.content.startsWith(PREFIX)) return;
  const args=m.content.slice(PREFIX.length).trim().split(/\s+/); const name=(args.shift()||'').toLowerCase(); if(!name)return;
  const cmd=client.commands.get(name); if(!cmd)return;
  try { await cmd.execute({ ...m, reply: (payload)=>m.reply(payload), options:{ getUser:()=>null, getRole:()=>null, getMember:()=>null, getInteger:()=>null }, isChatInputCommand:()=>false }); } catch(e){console.error(e); await m.reply({embeds:[leonisEmbed({title:'❌ Erro',description:'O Leonis não conseguiu executar esse comando.',color:COLORS.danger})]}); }
});
