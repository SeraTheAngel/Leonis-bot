...
import { isBlockedInvite, isRaidMessage } from './systems/security.js';
...
[actionCommand, ...utilityCommands, ...moderationCommands].forEach((command) => {
...
client.on(Events.GuildMemberAdd, async (member) => {
  ...
  joins.push(now);
  joinBursts.set(member.guild.id, joins);
  if (joins.length >= config.security.joinThreshold) {
    logLine(member.guild, `🚨 Possível raid detectada: ${joins.length} entradas em 15s.`);
    try { await member.guild.channels.cache.forEach((channel) => channel.isTextBased() && channel.setRateLimitPerUser(10, 'Leonis anti-raid')); } catch (error) { console.error('Anti-raid lockdown:', error); }
  }
});

client.on(Events.GuildMemberRemove, (member) => {
  recordMemberEvent(member.guild.id, 'leave', member.user?.id ?? member.id, member.user?.tag ?? member.id);
});

client.on(Events.MessageDelete, (message) => {
  if (message.guild) recordMessage(message, 'deleted');
});

client.on(Events.MessageUpdate, (oldMessage, newMessage) => {
  if (newMessage.guild && oldMessage.content !== newMessage.content) recordMessage(newMessage, 'edited', oldMessage.content);
});
