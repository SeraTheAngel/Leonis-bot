...
client.on(Events.MessageCreate, async m => {
  if (!m.guild || m.author.bot) return; const c = getGuildConfig(m.guild.id);
  if (c.security.blockInvites && isBlockedInvite(m.content)) { try { await m.delete(); } catch {} recordMessage(m, 'blocked-invite'); await sendLog(m.guild, { title: '🛡️ Convite bloqueado', description: `Mensagem de **${m.author.tag}** removida.\nCanal: ${m.channel}`, color: COLORS.warning }); return; }
  if (c.security.antiFlood) { const now = Date.now(); const key = `${m.guild.id}:${m.author.id}`; const e = (recentMessages.get(key) || []).filter(t => now - t < 8000); e.push(now); recentMessages.set(key, e); if (e.length >= c.security.floodThreshold) try { await m.member.timeout(10000, 'Leonis anti-flood'); } catch {} }
  recordMessage(m, 'created'); const keyword = findKeyword(m.guild.id, m.content);
  ...
  try { await handlePrefixMessage(m, client); } catch (e) { console.error('Prefix command error:', e); if (m.content.startsWith(process.env.PREFIX || 'l!')) await m.reply({ embeds: [leonisEmbed({ title: '❌ Erro', description: 'O Leonis não conseguiu executar esse comando.', color: COLORS.danger })] }).catch(() => {}); }
});

client.on(Events.GuildMemberAdd, async m => {
  recordMemberEvent(m.guild.id, 'join', m.user.id, m.user.tag); const c = getGuildConfig(m.guild.id);
  if (c.welcome.enabled && c.welcome.channelId) { const channel = m.guild.channels.cache.get(c.welcome.channelId) ?? await m.guild.channels.fetch(c.welcome.channelId).catch(() => null); if (channel?.isTextBased()) { const text = c.welcome.message.replaceAll('{user}', `${m}`).replaceAll('{server}', m.guild.name); await channel.send({ embeds: [leonisEmbed({ title: '👋 Bem-vindo(a)!', description: text, image: m.user.displayAvatarURL({ size: 512 }), color: COLORS.success })] }).catch(() => {}); } }
  await sendLog(m.guild, { title: '📥 Membro entrou', description: `**${m.user.tag}** entrou.\nID: \`${m.user.id}\``, color: COLORS.success });
  if (!c.security.antiRaid) return; const now = Date.now(); const e = (joinBursts.get(m.guild.id) || []).filter(t => now - t < 15000); e.push(now); joinBursts.set(m.guild.id, e);
...
client.on(Events.GuildMemberRemove, async m => { recordMemberEvent(m.guild.id, 'leave', m.user?.id ?? m.id, m.user?.tag ?? m.id); await sendLog(m.guild, { title: '📤 Membro saiu', description: `**${m.user?.tag ?? m.id}** saiu do servidor.`, color: COLORS.warning }); });

client.on(Events.MessageDelete, async m => {
  if (!m.guild) return; recordMessage(m, 'deleted'); let moderator = 'Não identificado';
  try { const logs = await m.guild.fetchAuditLogs({ type: AuditLogEvent.MessageDelete, limit: 5 }); const entry = logs.entries.find(e => e.target?.id === m.author?.id && Date.now() - e.createdTimestamp < 5000); if (entry?.executor) moderator = entry.executor.tag; } catch {}
  const content = m.content ? m.content.slice(0, 900) : '[texto não disponível no cache]'; const attachments = [...(m.attachments?.values?.() ?? [])].map(a => a.url).join('\n');
...
const destructiveEvents = [AuditLogEvent.ChannelDelete, AuditLogEvent.RoleDelete, AuditLogEvent.MemberBanAdd, AuditLogEvent.MemberKick, AuditLogEvent.ChannelCreate, AuditLogEvent.RoleCreate];
client.on(Events.GuildAuditLogEntryCreate, async (entry, guild) => {
  if (!destructiveEvents.includes(entry.action)) return;
  const executor = entry.executor; const triggered = await protectFromNuke(guild, entry.action, executor);
  await sendLog(guild, { title: triggered ? '🚨 ANTI-NUKE ATIVADO' : '📋 Ação administrativa', description: `**Ação:** \`${entry.action}\`\n**Executor:** ${executor?.tag ?? executor?.id ?? 'desconhecido'}${triggered ? '\n\n⚠️ Atividade destrutiva em massa detectada. Cargos do executor foram removidos.' : ''}`, color: triggered ? COLORS.danger : COLORS.normal });
});
