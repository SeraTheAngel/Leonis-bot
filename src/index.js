...
client.once(Events.ClientReady,bot=>console.log(`🦁 Leonis online como ${bot.user.tag} • ${client.commands.size} comandos`));
...
client.on(Events.InteractionCreate,async i=>{if(!i.isChatInputCommand())return;const c=client.commands.get(i.commandName);if(!c)return;try{await c.execute(i)}catch(e){console.error(e);const p={content:'❌ O Leonis tropeçou feio aqui. Tenta novamente.',ephemeral:true};if(i.replied||i.deferred)await i.followUp(p);else await i.reply(p)}});
